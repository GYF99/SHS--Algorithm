...
####有修改
    for i, cc in enumerate(C):
        for each_node in cc:
            # area[each_node - 1] += 1 << i  # node_id from 1 to n.    原内容
            area[each_node] += 1 << i
    kernels = []
    cnt = 0
    for i in range(len(C)):
        ...
        for i in range(len(G)):
            if (area[i] & mask) == mask:
                q.append((G.degree(weight=weight)[i], i))  ####改i+1为i
        q.sort()
        q.reverse()
        for i in range(
...
def get_structural_holes_MaxD(G,G_index, index_of_node, node_of_index, k, C: List[frozenset]):
    ...
    # G_index, index_of_node, node_of_index = G.to_index_node_graph(begin_index=0)   #该行为原代码
    # G_index, index_of_node, node_of_index = to_index_node_graph(G, begin_index=0)
    C_index = []
    for cmnt in C:
        cmnt_index = []
        for node in cmnt:
