...
def sort_nodes_by_degree(G, weight="weight"):
    ...
    for node, degree in sorted(
        ...
        #G.degree(weight=weight).items(), key=lambda x: x[1], reverse=True  #原代码
    ):
...
def procedure1(G, c=1.0):
    ...
    for component in components:
        #component_subgraph = G.nodes_subgraph(from_nodes=list(component))  #原
        nodes_of_subgraph = list(component)
        component_subgraph = G.subgraph(nodes_of_subgraph)
